...
from ..eip7069_extcall.spec import EXTCALL_FAILED, EXTCALL_REVERT, LEGACY_CALL_FAILURE
from .helpers import (
    aborting_container,
    slot_call_or_create,
    slot_call_result,
...
from .spec import EOFCREATE_FAILED
...
def test_initcode_revert(state_test: StateTestFiller, pre: Alloc, revert: bytes):
    ...
    post = {
        contract_address: Account(
            storage={
                slot_create_address: EOFCREATE_FAILED,
...
def test_static_flag_eofcreate(
    ...
    post = {
        calling_address: Account(
            storage={
                slot_call_result: EXTCALL_FAILED
...
def test_eof_eofcreate_msg_depth(
    ...
    calling_storage = {
        slot_max_depth: 1024,
        slot_code_worked: value_code_worked,
        slot_call_result: EXTCALL_REVERT if who_fails == magic_value_call else EOFCREATE_FAILED,
